...
class TestLoginFunctions(unittest.TestCase):
    ...
    def test_internal_get_call(self):
        # setup
        uri = "/login"
        self.response.content = open("tests/data/login_challenge_response.xml").read()
        # test
        self.api._get(uri)
        # validate
        self.assertTrue(
    ...
    def test_login_get_failed_challenge(self):
        ...
        self.response.content = open(
            "tests/data/login_challenge_invalid_account_response.xml"
        ).read()
        # validate
    ...
    def test_login_send_challenge_response(self):
        ...
        self.response.content = open(
            "tests/data/login_challenge_response_response.xml"
        ).read()
        # test
    ...
    def test_login_send_challenge_failed_response(self):
        ...
        self.api.secret = self.secret
        self.response.content = open(
            "tests/data/login_invalid_challenge_response_response.xml"
        ).read()
        # validate
        self.assertRaises(
            ApiException, self.api._login_response, ("92dc06bbb703f14354fdfbede9b62ff9")
        )
